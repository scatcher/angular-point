import {CacheService, DataService, UtilityService, FieldService, DecodeService, EncodeService} from '../services';
import {List, Query, IndexedCache, ListItem, User} from '../factories';
import {APConfig, IUserPermissionsObject, BasePermissionObject} from '../constants';
import {IUninstantiatedList} from './apListFactory';
...
export class Model {
    ...
    constructor(config: IUninitializedModel) {
        ...
        CacheService.registerModel(this);
     ...
     * @param {object} [options.indexedCache=IndexedCache.create({})] Optionally place new item in a specified cache.
     ...
     * @returns {Promise<T>} A promise which when resolved will returned the newly created list item from there server.
    ...
    addNewItem<T extends ListItem<any>>(entity: ListItem<T>, {
        ...
        indexedCache = new IndexedCache({}),
        valuePairs = []
        }: ICreateListItemOptions<T> = {}): Promise<T> {
        ...
        return DataService.serviceWrapper(config)
            .then((response) => {
                ...
                let indexedCache = DecodeService.processListItems<T>(this, config, response, config);

                /** Last listItem in cache is new because it has the highest id */
                let newListItem: ListItem<T> = indexedCache.last();

                /** Optionally broadcast change event */
                UtilityService.registerChange(this, 'create', newListItem.id);
     ...
     * @returns {Promise<IndexedCache<T>>} Promise that when resolves with an IndexedCache of list items which inherit from ListItem and
     ...
     * <pre>
     * projectModel.executeQuery('MyCustomQuery').then((entities) => {
     *      //We now have a reference to IndexedCache of entities stored in the local cache
     *      //These inherit from the ListItem prototype as well as the Project prototype on the model
     *      this.subsetOfProjects = entities;
     *  });
     * </pre>
     */
    executeQuery<T extends ListItem<any>>(queryName?: string): Promise<IndexedCache<T>> {
        var model = this;
     ...
     * @returns {Promise<Model>} Promise that is resolved with the extended model.
     */
    extendListMetadata(): Promise<Model> {
        var model = this;
        ...
        let promise = new Promise((resolve, reject) => {
            ...
            if (!model.deferredListDefinition) {
                ...
                model.deferredListDefinition = promise;
                ...
                Promise.all(promiseArray)
                    .then((resolvedPromises) => {
                        DecodeService.extendListMetadata(model, resolvedPromises[0]);
                        resolve(model);
                    })
     ...
     * </pre>
     */
    getAllListItems<T extends ListItem<any>>(): Promise<IndexedCache<T>> {
        var model = this;
    ...
    getCachedEntities<T extends ListItem<any>>(): IndexedCache<T> {
        var model = this;
        return CacheService.getCachedEntities<T>(model.getListId());
    }
    ...
    getCachedEntity<T extends ListItem<any>>(listItemId: number): T {
        var model = this;
        return CacheService.getCachedEntity<T>(model.getListId(), listItemId);
    }
     ...
     * </pre>
     */
    getListItemById<T extends ListItem<any>>(listItemId: number, options?: Object): Promise<T> {
        var model = this,
            /** Unique Query Name */
    ...
    resolvePermissions(): IUserPermissionsObject {
        ...
        if (list && list.permissions) {
            ...
            console.error('Attempted to resolve permissions of a model that hasn\'t been initialized.', model);
