...
export function SPServicesCore(apWebServiceOperationConstants: IWebServiceOperationConstants, apWebServiceService: WebServiceService) {
    ...
    const encodeOptionList = ['listName', 'description']; // Used to encode options which may contain special characters
    ...
    function generateXMLComponents(options: SPServicesOptions) {
        ...
        const service = apWebServiceOperationConstants[opt.operation][0];
    ...
    function wrapNode(n, v) {
        const thisValue = typeof v !== 'undefined' ? v : '';
