 ...
 */
export class DecodeServiceClass {
    ...
    createListItemProvider<T extends ListItem<any>>(model: Model, query: Query<T>, indexedCache: IndexedCache<T>): (rawObject: IUninstantiatedExtendedListItem<T>) => ListItem<T> {
        return (rawObject: IUninstantiatedExtendedListItem<T>) => {
            ...
            if (indexedCache.has(rawObject.id)) {
                ...
                CacheService.registerEntity<T>(listItem, indexedCache);
            }
    ...
    extendFieldDefinitionsFromXML(fieldDefinitions: IFieldDefinition[], responseXML: Element): IFieldDefinition[] {
        ...
        for (let xmlField: Element of filteredNodes) {
            ...
            if (fieldDefinition) {
                ...
                this.extendObjectWithXMLAttributes(xmlField, fieldDefinition, XMLFieldAttributeTypes);
    ...
    jsLookup(str: string): Lookup<any> {
        if (str.length === 0) {
            return null;
        } else {
            //Send to constructor
    ...
    jsLookupMulti(str: string): Lookup<any>[] {
        if (str.length === 0) {
            return [];
        } else {
            ...
            for (let i = 0; i < thisLookupMulti.length; i = i + 2) {
                ...
                if (thisLookupMulti[i]) {
                    let thisLookup = this.jsLookup(thisLookupMulti[i] + ';#' + thisLookupMulti[i + 1]);
    ...
    parseFieldVersions(responseXML: Element, fieldDefinition: FieldDefinition): FieldVersionCollection {
        ...
        _.each(xmlVersions, (xmlVersion, index) => {
             ...
             */
            let utcDate = this.parseStringValue(xmlVersion.getAttribute('Modified'), 'DateTime');

            /** Parse the xml and create a representation of the version as a js object */
            let editor = this.parseStringValue(xmlVersion.getAttribute('Editor'), 'User');
            ...
            let value = this.parseStringValue(xmlVersion.getAttribute(fieldDefinition.staticName), fieldDefinition.objectType);
     ...
     */
    parseStringValue(str: string, objectType?: string): any {
        ...
        switch (objectType) {
            ...
            case 'DateTime':
                ...
                break;
            case 'Lookup':
                colValue = this.jsLookup(unescapedValue);
                break;
            case 'User':
                colValue = this.jsUser(unescapedValue);
                break;
            case 'LookupMulti':
                colValue = this.jsLookupMulti(unescapedValue);
                break;
            case 'UserMulti':
    ...
    parseXmlEntity<T extends ListItem<any>>(xmlEntity: Element, { mapping, includeAllAttrs = false, removeOws = true }: IParseXmlEntityOptions) {
        ...
        _.each(mapping, (fieldDefinition) => {
            entity[fieldDefinition.mappedName] = FieldService.getDefaultValueForType(fieldDefinition.objectType);
        });
...
export let DecodeService = new DecodeServiceClass();
