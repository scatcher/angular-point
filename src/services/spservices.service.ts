import {SPServicesDefaults, WebServiceOperationConstants, action, xmlns} from '../constants';
import {encodeXml, wrapNode} from '../services/encode.service';
...
export function generateXMLComponents(options: SPServicesOptions) {
    ...
    soapEnvelope.opheader = `<${opt.operation} xmlns="${xmlns(service)}" >`;
    SOAPAction = action(service);
