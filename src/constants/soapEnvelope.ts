...
export class SOAPEnvelope implements ISOAPEnvelope {
    ...
    addToPayload(opt, paramArray) {
        _.each(paramArray, (param, index: number) => {
            // the parameter name and the option name match
            if (typeof param === 'string') {
                this.payload += EncodeService.wrapNode(param, opt[param]);
                // the parameter name and the option name are different
            } else if (_.isArray(param) && param.length === 2) {
                this.payload += EncodeService.wrapNode(param[0], opt[param[1]]);
                ...
                this.payload += ((opt[param.name] === undefined) || (opt[param.name].length === 0)) ? '' : EncodeService.wrapNode(param.name, opt[param.name]);
