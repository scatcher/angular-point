 ...
 * @returns {string}
 */
function choiceMultiToString(choices) {
    var str = '';
    var delim = ';#';
    if (choices.length > 0) {
...
exports.choiceMultiToString = choiceMultiToString;
/**
 * @ngdoc function
...
exports.createValuePair = createValuePair;
/**
 * @ngdoc function
...
function encodeValue(fieldType, value) {
    ...
    if (value !== '' && !lodash_1.isUndefined(value) && !lodash_1.isNull(value)) {
        switch (fieldType) {
            case 'Lookup':
            case 'User':
                if (value.lookupId) {
 ...
 */
function generateValuePairs(fieldDefinitions, listItem) {
    var _this = this;
    var pairs = fieldDefinitions
        .filter(function (field) {
        return lodash_1.has(listItem, field.mappedName);
    })
        .map(function (field) {
        return _this.createValuePair(field, listItem[field.mappedName]);
    });
    return pairs;
}
exports.generateValuePairs = generateValuePairs;
/**
 * @ngdoc function
 ...
 * @returns {string}
 */
function generateViewFieldsXML(fieldDefinitions) {
    /** Open viewFields */
...
exports.generateViewFieldsXML = generateViewFieldsXML;
/**
 * @ngdoc function
...
function stringifySharePointDate(date) {
    var jsDate;
    if (!lodash_1.isDate(date) && lodash_1.isString(date) && date.split('-').length === 3) {
    ...
    dateString += '-';
    dateString += utility_service_1.doubleDigit(jsDate.getMonth() + 1);
    dateString += '-';
    dateString += utility_service_1.doubleDigit(jsDate.getDate());
    dateString += 'T';
    dateString += utility_service_1.doubleDigit(jsDate.getHours());
    dateString += ':';
    dateString += utility_service_1.doubleDigit(jsDate.getMinutes());
    dateString += ':';
    dateString += utility_service_1.doubleDigit(jsDate.getSeconds());
    dateString += 'Z-';
    if (!savedTimeZone) {
    ...
    dateString += utility_service_1.doubleDigit(savedTimeZone);
    dateString += ':00';
    return dateString;
}
exports.stringifySharePointDate = stringifySharePointDate;
/**
 * @ngdoc function
...
function stringifySharePointMultiSelect(multiSelectValue, idProperty, valueProperty) {
    ...
    lodash_1.each(multiSelectValue, function (lookupObject, iteration) {
...
exports.stringifySharePointMultiSelect = stringifySharePointMultiSelect;
/**
 * @ngdoc function
