...
exports.convertEffectivePermMask = convertEffectivePermMask;
/**
 * @ngdoc function
...
function registerChange(model, changeType, listItemId) {
    ...
    if (model.sync && lodash_1.isFunction(model.sync.registerChange)) {
...
function resolvePermissions(permissionsMask) {
    ...
    if (permissionSet.FullMask) {
        lodash_1.each(permissionSet, function (perm, key) {
...
function stringifyXML(xml) {
    var str;
    if (lodash_1.isObject(xml)) {
        str = xmlToString(xml).replace(/\s+/g, ' ');
    }
    else if (lodash_1.isString(xml)) {
        str = xml;
    }
    return str;
}
